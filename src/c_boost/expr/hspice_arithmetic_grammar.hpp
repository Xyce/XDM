...
#ifndef HSPICE_ARITHMETIC_GRAMMAR_H
#define HSPICE_ARITHMETIC_GRAMMAR_H
...
struct HSPICEArithmeticGrammar :
    boost::spirit::qi::grammar<Iterator, ast_common::root(), boost::spirit::ascii::space_type>
{
    ...
    qi::rule<Iterator, std::string(), ascii::space_type> any, eq, gt, gtoe, func_begin, func_identifier, func_end, func_separator, identifier,
         ineq, logical_and, logical_or, lt, ltoe, math_expression, numeric, number, parenthetical_math_expression, raw_first_identifier, raw_identifier,
         pi, natural_logarithm, logarithm10, exponent, cosine, sine, tangent, agauss, built_in_function, built_in_function_identifier,
         square_root, aunif, arccosine, arcsine, arctangent, maximum, minimum, integer, absolute_value, signum, absolute_power, signed_power,
         hyperbolic_cosine, hyperbolic_sine, hyperbolic_tangent, ternary_condition, ternary_expression, add, subtract, multiply, divide,
         power_double_asterisk, power_caret;

    qi::rule<Iterator> white_space;

    HSPICEArithmeticGrammar() : HSPICEArithmeticGrammar::base_type(start)
    {
        using qi::char_;
        using qi::hold;
        using qi::lit;
        ...
        number = 
            ...
            hold[numeric >> -(char_(".") >> -numeric) >> no_case[char_("afpnumkxg")]] |
            hold[numeric >> -(char_(".") >> -numeric)] |
            hold[char_(".") >> numeric >> no_case[char_("e") >> -(char_("-") | char_("+")) >> numeric]] | 
            hold[char_(".") >> numeric >> no_case[char_("afpnumkxg")]] |
            hold[char_(".") >> numeric]
            ;
       ...
       built_in =
           hold[pi] | hold[built_in_function]
           ;
